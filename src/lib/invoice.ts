...
export const generateInvoicePDF = (data: InvoiceData) => {
  ...
  doc.setFontSize(10);
  doc.text(`Invoice No: #INV-${data.orderId.slice(0, 8).toUpperCase()}`, 14, 62);
  ...
  const tableData = data.items.map((item, index) => {
    ...
    // Assuming 18% GST (9% CGST + 9% SGST for local, 18% IGST for interstate)
    // For now, we show total inclusive of GST as provided in total_amount
    return [
      index + 1,

...
export default async function ProfilePage() {
  ...
  const firstName = profile?.first_name || (typeof profile?.full_name === "string" ? profile.full_name.split(" ")[0] : "") || "";
  const lastName = profile?.last_name || (typeof profile?.full_name === "string" ? profile.full_name.split(" ").slice(1).join(" ") : "") || "";
  const companyName = profile?.company_name || "Independent Engineer";
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        <Card className="lg:col-span-2 rounded-[32px] border-border/40 bg-white/50 backdrop-blur-xl shadow-2xl shadow-primary/5">
          ...
          <CardContent className="p-8 space-y-8">
            ...
            <ProfileForm defaultFirstName={firstName} defaultLastName={lastName} defaultCompanyName={companyName} />
          </CardContent>
        </Card>
